...
def G729_SS_QCCCN(file):
    ...
    # c1和c2分别用于统计data中每个帧的第一个和第四个码字(下一个语音帧的第一个码字)的出现次数。
    # a和p分别用于计算码字之间的转移概率
    ...
    # 修改相应的代码
    num_codewords = 10  # 每一个语音帧包含的码字数量
    for i in range(len(data) - 1):
        data1 = data[i]
        data2 = data[i + 1]
        c1[data1[0]] += 1
        c2[data1[num_codewords]] += 1
        a[data1[0], data2[0]] += 1
        p[data1[num_codewords], data1[num_codewords+1]] += 1
...
def main(positive_data_folder, negative_data_folder, t_positive_data_folder, t_negative_data_folder, result_folder):
    ...
    # 进行主成分分析（PCA），以降低数据的维度并找到数据中的主要变化方向
    ...
    with open(os.path.join(result_folder, "pca.pkl"), "wb") as f:      # 为什么pca.pkl文件后面没有用到？
        pickle.dump(pca, f)
    ...
    for i in tqdm(range(num_train_files)):
        ...
        X.append(pca.transform(new_feature.reshape(1, -1)))
        Y.append(1)
    X = np.row_stack(X)  # 将存储在列表 X 中的特征向量沿垂直方向叠加成一个大的特征矩阵
    ...
    for i in tqdm(range(num_test_files)):
        ...
        Y.append(1)
    X = np.row_stack(X)
    Y_predict = clf.predict(X)      # 使用已经训练好的SVM分类器 clf 对测试数据进行分类预测，得到预测的类别标
