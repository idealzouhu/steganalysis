...
if __name__ == '__main__':
    ...
    # 准备训练数据的部分，主要包括数据加载、数据打乱和划分数据集
    train_files = [(item, folder["class"]) for folder in TRAIN_FOLDERS for item in get_file_list(folder["folder"])]  # 将每个文件的路径与其所属的类别存储在列表中
    random.shuffle(train_files)
    train_samples_x = [(parse_sample(item[0])) for item in train_files]  # 加载每个训练样本的特征数据
    train_samples_y = [item[1] for item in train_files]    # 提取每个样本的类别标签
    np_train_samples_x = np.asarray(train_samples_x)
    np_train_samples_y = np.asarray(train_samples_y)
    file_num = len(train_files)
    sub_file_num = int(file_num / 5)    # 将训练数据划分为训练集和验证集，其中验证集占总样本数的1/5
